...
class FilterService {
  ...
  void _initOverlayListener() {
    if (_messageHandlerRegistered) return;
    ...
    _overlayMessageChannel.setMessageHandler((message) async {
      ...
      } else if (decodedEvent is Map && decodedEvent['type'] == 'refresh_filters') {
  ...
  Future<void> saveFilters(BotFilters newFilters) async {
    try {
      ...
      // Notificar a otros isolates para que recarguen de SharedPreferences
  ...
  Future<void> toggleBot(bool active) async {
    ...
    await FlutterOverlayWindow.shareData(active ? 'bot_activated' : 'bot_deactivated');
  }
